...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  try {
    ...
    console.error("Failed to load SEO metadata for about subpage:", err);
  }

  const title = matchedSeo?.meta_title || `${subpage.split("-").map(w => w.charAt(0).toUpperCase() + w.slice(1)).join(" ")} | Jivanjor`;
  const description = matchedSeo?.meta_description || "Learn more about Jivanjor woodworking adhesives, research R&D facilities, performance quality, TVCs, and our presence.";
  const canonical = matchedSeo?.canonical_url || `https://jivanjor.com/about/${subpage}`;
