...
export default function AdminSitemapManagerPage() {
  const [config, setConfig] = useState<SitemapConfig>(() => getLocalSitemapConfig());
  const [isSaving, setIsSaving] = useState(false);
  const [isSyncing, setIsSyncing] = useState(false);
  const [toastMessage, setToastMessage] = useState("");

  // New Section modal / inline form state
  const [newSectionTitle, setNewSectionTitle] = useState("");
  const [newSectionDesc, setNewSectionDesc] = useState("");
  const [showAddSectionForm, setShowAddSectionForm] = useState(false);

  // New Link inline form state per section (sectionId -> { title, href, description })
  const [newLinkData, setNewLinkData] = useState<{ [sectionId: string]: { title: string; href: string; description: string } }>({});
  ...
  const handleReset = () => {
    if (confirm("Are you sure you want to reset sitemap to default settings?")) {
  ...
  const handleUpdateSectionHeader = (sectionId: string, title: string, description: string) => {
    setConfig((prev) => ({
      ...prev,
      sections: prev.sections.map((sec) =>
      ...
      <div className="space-y-8 pb-12">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-border pb-6">
          <div>
            ...
            <p className="text-sm text-foreground/60 mt-1">
              Create, edit, and organize the sections and links displayed on the public website sitemap.
            </p>
          </div>
          ...
          <div className="flex items-center gap-3">
            <Link
              href="/sitemap"
              target="_blank"
              className="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-surface border border-border text-foreground font-semibold text-xs sm:text-sm hover:border-primary/50 hover:text-primary transition-all"
            >
              <Globe className="w-4 h-4" />
              <span>Preview Public Sitemap</span>
        ...
        {showAddSectionForm && (
          ...
          >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className="block text-xs font-semibold text-foreground/70 mb-1">Section Title</label>
                <input
                  type="text"
              ...
              <div>
                <label className="block text-xs font-semibold text-foreground/70 mb-1">Section Description (Optional)</label>
                <input
                  type="text"
                  placeholder="e.g., Custom application guides & specialized bonding adhesives."
            ...
            <div className="flex justify-end gap-3 pt-2">
              ...
              <button
                type="submit"
                className="px-4 py-2 rounded-xl bg-primary text-white font-bold text-xs shadow-xs hover:bg-primary/90"
              >
                Create Section
              </button>
            </div>
            ...
            >
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 mb-4 border-b border-border">
                <div className="flex-1 space-y-2">
                  <div className="flex items-center gap-3">
                    <input
                      ...
                      value={section.title}
                      onChange={(e) =>
                        handleUpdateSectionHeader(section.id, e.target.value, section.description || "")
                      }
                      className="font-bold text-base bg-transparent border-b border-transparent hover:border-border focus:border-primary text-foreground focus:outline-none transition-colors px-1"
                    />
                    <span className="text-xs text-foreground/40 font-mono">({section.links.length} links)</span>
                  </div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      handleUpdateSectionHeader(section.id, section.title, e.target.value)
                    }
                ...
                <div className="flex items-center gap-2 shrink-0">
                  ...
                  <button
                    ...
                    title={section.isVisible ? "Hide section on public sitemap" : "Show section on public sitemap"}
              ...
              <div className="space-y-3 mb-6">
                {section.links.length === 0 ? (
                  <p className="text-xs text-foreground/40 italic">No links in this section. Add one below.</p>
                ) : (
                  section.links.map((link) => (
                    <div
                      key={link.id}
                      className="flex items-center justify-between p-3 rounded-xl bg-background border border-border/70 text-xs"
                    >
                      <div className="flex-1 grid grid-cols-1 sm:grid-cols-3 gap-2 pr-4">
                        ...
                        <span className="font-mono text-foreground/50 truncate">{link.href}</span>
                        <span className="text-foreground/60 truncate">{link.description || "—"}</span>
                      </div>

                      <button
                        onClick={() => handleDeleteLink(section.id, link.id)}
                        className="p-1.5 text-foreground/40 hover:text-red-500 transition-colors cursor-pointer"
                        title="Delete link"
                      >
              ...
              <div className="p-3 bg-background/60 border border-border/50 rounded-xl space-y-2">
